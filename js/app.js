const API_BASE_URL = "https://logistics-scanned-treo-college.trycloudflare.com";
        ...
        <tr>
            ...
            <td class="text-nowrap">
                ${user.id !== currentUser.id ? `
                    <button class="btn btn-sm btn-outline-warning" onclick="showPasswordReset(${user.id}, '${escapeJs(user.usuario)}', '${escapeJs(user.nombre)}')">
                        <i class="bi bi-key"></i> Restablecer
                    </button>` : '<span class="text-muted small">Cuenta actual</span>'}
            </td>
        </tr>`).join("");

    content.innerHTML = `
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
            <div><h1 class="h2 mb-1">Usuarios</h1><p class="text-muted mb-0">Administración individual y carga masiva.</p></div>
            <div class="d-flex align-items-center gap-2">
                <span class="badge text-bg-primary fs-6">${users.length} usuarios</span>
                <button class="btn btn-primary" onclick="showNewUserForm()"><i class="bi bi-person-plus"></i> Nuevo usuario</button>
            </div>
        </div>
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-body">
                ...
                <form id="importUsersForm" class="row g-3">
                    ...
                    <div class="col-12"><button id="importButton" class="btn btn-outline-primary"><i class="bi bi-upload"></i> Validar e importar</button></div>
                </form>
            </div>
        </div>
