const API_BASE_URL = "https://committee-pads-thereof-satisfactory.trycloudflare.com";
...
function hasPermission(key){return Array.isArray(currentUser?.permisos)&&currentUser.permisos.includes(key)}
function hasAnyPermission(...keys){return keys.some(hasPermission)}
function roleDisplayName(item){return item?.rol_nombre||item?.rol||""}
...
async function loadAuthenticatedUser() {
    ...
    usersNav.classList.toggle("d-none", !hasAnyPermission("usuarios.ver","usuarios.crear","usuarios.reportes","direcciones.administrar"));
    supportNav.classList.toggle("d-none", !hasAnyPermission("catalogos.administrar","permisos.administrar"));
    document.querySelector('[data-module="usuarios-registro"]')?.classList.toggle("d-none",!hasPermission("usuarios.crear"));
    document.querySelector('[data-module="usuarios-alta"]')?.classList.toggle("d-none",!hasPermission("usuarios.crear"));
    document.querySelector('[data-module="usuarios-consulta"]')?.classList.toggle("d-none",!hasPermission("usuarios.ver"));
    document.querySelector('[data-module="usuarios-reporte"]')?.classList.toggle("d-none",!hasPermission("usuarios.reportes"));
    document.querySelector('[data-module="direcciones"]')?.classList.toggle("d-none",!hasPermission("direcciones.administrar"));
...
async function renderDashboard() {
    ...
    content.innerHTML = `
        <div class="dashboard-welcome mb-4"><div class="position-relative" style="z-index:1"><div class="small text-uppercase opacity-75 fw-semibold mb-1">Intranet corporativa</div><h1 class="h2 mb-1">Hola, ${escapeHtml(currentUser.nombre)}</h1><p class="mb-0 opacity-75">Consulta avisos, políticas, comunicados y documentos internos.</p>${hasPermission("avisos.administrar") ? '<button class="btn btn-light btn-sm mt-3" onclick="showNoticeAdmin()"><i class="bi bi-megaphone me-1"></i> Administrar aviso</button>' : ""}</div></div>
        ${notice}
        <div class="row g-3">
...
async function openPublicationInbox(type) {
    ...
    const canPublish = hasPermission("publicaciones.crear");
...
async function renderSupport(){
 if(!hasAnyPermission("catalogos.administrar","permisos.administrar")){renderError("No tienes permiso para administrar este módulo.");return;}
...
async function saveRolePermissions(roleId){const permiso_ids=[...document.querySelectorAll(`.rp-${roleId}:checked`)].map(x=>Number(x.value));try{await apiFetch(`/roles/${roleId}/permisos`,{method:"PUT",body:JSON.stringify({permiso_ids})});alert("Permisos actualizados.");}catch(error){alert(error.message)}}
