const API_BASE_URL = "https://him-cookbook-style-stat.trycloudflare.com";
...
async function renderUserRegistration() {
    ...
    content.innerHTML = `
        <div class="mb-4"><h1 class="h2 mb-1">Registro masivo</h1><p class="text-muted mb-0">Importa usuarios mediante el layout CSV UTF-8.</p></div>
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                <h2 class="h5">Importar layout CSV</h2>
                <p class="text-muted">Columnas: nombre, usuario, correo, rol y activo.</p>
                ...
                <form id="importUsersForm" class="row g-3">
                    ...
                    <div class="col-12"><button id="importButton" class="btn btn-primary"><i class="bi bi-upload"></i> Validar e importar</button></div>
                </form>
            </div>
        </div>`;
...
async function renderUserAlta() {
    ...
    content.innerHTML = `
        <div class="mb-4"><h1 class="h2 mb-1">Alta individual</h1><p class="text-muted mb-0">Crea una cuenta con contraseña temporal.</p></div>
        <div id="userActionArea"></div>`;
    showNewUserForm();
}
...
async function renderUserConsultation() {
    ...
    const rows = users.map(user => `
        <tr>
            ...
            <td class="text-nowrap">${user.id !== currentUser.id ? `<button class="btn btn-sm btn-outline-warning" onclick="showPasswordReset(${user.id}, '${escapeJs(user.usuario)}', '${escapeJs(user.nombre)}')"><i class="bi bi-key"></i> Restablecer</button>` : '<span class="text-muted small">Cuenta actual</span>'}</td>
        </tr>`).join("");
    content.innerHTML = `
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
            <div><h1 class="h2 mb-1">Consulta de usuarios</h1><p class="text-muted mb-0">Revisa las cuentas y restablece contraseñas.</p></div>
            <span class="badge text-bg-primary fs-6">${users.length} usuarios</span>
        </div>
        <div id="userActionArea"></div>
        <div class="card shadow-sm border-0"><div class="table-responsive"><table class="table table-striped align-middle mb-0"><thead><tr><th>Usuario</th><th>Nombre</th><th>Correo</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
}
...
async function renderUserReport() {
    ...
    const roleCounts = activeUsers.reduce((totals, user) => {
        totals[user.rol] = (totals[user.rol] || 0) + 1;
        return totals;
    }, {});
    const rows = activeUsers.map(user => `<tr><td>${escapeHtml(user.usuario)}</td><td>${escapeHtml(user.nombre)}</td><td>${escapeHtml(user.correo)}</td><td>${escapeHtml(user.rol)}</td><td>Activo</td></tr>`).join("");
    const roleSummary = Object.entries(roleCounts).sort().map(([role, total]) => `<span class="badge text-bg-secondary me-2 mb-2">${escapeHtml(role)}: ${total}</span>`).join("");
    content.innerHTML = `
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
            <div><h1 class="h2 mb-1">Reporte de usuarios activos</h1><p class="text-muted mb-0">Consulta sencilla de las cuentas activas registradas.</p></div>
            <button id="exportUsersButton" class="btn btn-success"><i class="bi bi-file-earmark-excel"></i> Exportar a Excel</button>
        </div>
        <div class="row g-3 mb-4">
            <div class="col-md-4"><div class="metric-card p-4"><div class="text-muted">Usuarios activos</div><div class="display-5 fw-bold">${activeUsers.length}</div></div></div>
            <div class="col-md-8"><div class="metric-card p-4"><div class="text-muted mb-2">Distribución por rol</div>${roleSummary || '<span class="text-muted">Sin datos</span>'}</div></div>
        </div>
        <div id="reportAlert"></div>
        <div class="card shadow-sm border-0"><div class="table-responsive"><table class="table table-striped align-middle mb-0"><thead><tr><th>Usuario</th><th>Nombre</th><th>Correo</th><th>Rol</th><th>Estado</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
...
async function exportActiveUsersExcel() {
    const button = document.getElementById("exportUsersButton");
    const alertBox = document.getElementById("reportAlert");
    button.disabled = true;
    button.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Generando Excel';
    try {
        const response = await fetch(`${API_BASE_URL}/usuarios/reporte.xlsx`, {
        ...
        link.href = url;
        link.download = "usuarios_activos.xlsx";
        document.body.appendChild(link);
        link.click();
        link.remove();
        URL.revokeObjectURL(url);
        alertBox.innerHTML = '<div class="alert alert-success">Reporte Excel generado correctamente.</div>';
    } catch (error) {
        alertBox.innerHTML = `<div class="alert alert-danger">${escapeHtml(error.message)}</div>`;
    } finally {
        button.disabled = false;
        button.innerHTML = '<i class="bi bi-file-earmark-excel"></i> Exportar a Excel';
    }
}
    ...
    document.getElementById("userActionArea").innerHTML = `
        <div class="card shadow-sm border-0 mb-4"><div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-start">
                <div><h2 class="h4 mb-1">Nuevo usuario</h2><p class="text-muted">La contraseña será temporal y deberá cambiarse al iniciar sesión.</p></div>
                <button class="btn-close" onclick="closeUserAction()" aria-label="Cerrar"></button>
            </div>
            <div id="newUserAlert"></div>
            <form id="newUserForm" class="row g-3">
                <div class="col-md-6"><label class="form-label">Nombre completo</label><input id="newUserName" class="form-control" minlength="3" required></div>
                <div class="col-md-3"><label class="form-label">Usuario</label><input id="newUsername" class="form-control" pattern="[a-z0-9._-]{3,50}" placeholder="jperez" required></div>
                <div class="col-md-3"><label class="form-label">Rol</label><select id="newUserRole" class="form-select" required><option value="promotor">Promotor</option><option value="consulta">Consulta</option><option value="supervisor">Supervisor</option><option value="rh">RH</option><option value="administrador">Administrador</option></select></div>
                <div class="col-md-7"><label class="form-label">Correo</label><input id="newUserEmail" type="email" class="form-control" required></div>
                <div class="col-md-5"><label class="form-label">Contraseña temporal</label><input id="newUserPassword" type="password" minlength="10" class="form-control" required></div>
                <div class="col-12"><button class="btn btn-primary" type="submit"><i class="bi bi-person-check"></i> Crear usuario</button></div>
            </form>
        </div></div>`;
    document.getElementById("newUserForm").addEventListener("submit", createSingleUser);
    document.getElementById("userActionArea").scrollIntoView({behavior: "smooth"});
}

async function createSingleUser(event) {
    event.preventDefault();
    const alertBox = document.getElementById("newUserAlert");
    try {
        await apiFetch("/usuarios", {method: "POST", body: JSON.stringify({
            nombre: document.getElementById("newUserName").value,
            usuario: document.getElementById("newUsername").value.toLowerCase().trim(),
            correo: document.getElementById("newUserEmail").value.toLowerCase().trim(),
            rol: document.getElementById("newUserRole").value,
            activo: true,
            password_temporal: document.getElementById("newUserPassword").value
        })});
        await renderUserConsultation();
        document.getElementById("userActionArea").innerHTML = '<div class="alert alert-success">Usuario creado. Entrega la contraseña temporal de forma segura.</div>';
    } catch (error) {
        alertBox.innerHTML = `<div class="alert alert-danger">${escapeHtml(error.message)}</div>`;
    }
}

function showPasswordReset(id, username, name) {
    document.getElementById("userActionArea").innerHTML = `
        <div class="card shadow-sm border-0 mb-4"><div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-start">
                <div><h2 class="h4 mb-1">Restablecer contraseña</h2><p class="text-muted mb-0">${escapeHtml(name)} (${escapeHtml(username)})</p></div>
                <button class="btn-close" onclick="closeUserAction()"></button>
            </div>
            <div class="alert alert-warning mt-3">La contraseña actual dejará de funcionar. El usuario deberá cambiar la nueva contraseña temporal en el siguiente acceso.</div>
            <div id="resetPasswordAlert"></div>
            <form id="resetPasswordForm" class="row g-3">
                <div class="col-md-7"><label class="form-label">Nueva contraseña temporal</label><input id="resetTemporaryPassword" type="password" minlength="10" class="form-control" required></div>
                <div class="col-md-5 d-flex align-items-end"><button class="btn btn-warning w-100" type="submit"><i class="bi bi-key-fill"></i> Confirmar restablecimiento</button></div>
            </form>
        </div></div>`;
    document.getElementById("resetPasswordForm").addEventListener("submit", event => resetUserPassword(event, id, username));
    document.getElementById("userActionArea").scrollIntoView({behavior: "smooth"});
}

async function resetUserPassword(event, id, username) {
    event.preventDefault();
    if (!confirm(`¿Restablecer la contraseña de ${username}?`)) return;
    const alertBox = document.getElementById("resetPasswordAlert");
    try {
        await apiFetch(`/usuarios/${id}/restablecer-password`, {method: "POST", body: JSON.stringify({
            password_temporal: document.getElementById("resetTemporaryPassword").value
        })});
        alertBox.innerHTML = '<div class="alert alert-success">Contraseña restablecida. El usuario deberá cambiarla en el siguiente acceso.</div>';
