...
async function renderDashboard() {
    ...
    content.innerHTML = `
        <div class="mb-4"><h1 class="h2 mb-1">Hola, ${escapeHtml(currentUser.nombre)}</h1><p class="text-muted">Consulta avisos y documentos internos.</p>${["administrador","rh"].includes(currentUser.rol) ? '<button class="btn btn-sm btn-outline-primary mt-2" onclick="showNoticeAdmin()"><i class="bi bi-megaphone"></i> Administrar aviso emergente</button>' : ""}</div>
        ${notice}
        <div class="row g-3">
...
function showGeneralNotice(aviso) {
    ...
    document.body.insertAdjacentHTML("beforeend", `<div id="generalNoticeModal" class="notice-overlay"><div class="notice-dialog"><button class="btn-close notice-close" onclick="document.getElementById('generalNoticeModal').remove()"></button><div class="notice-icon"><i class="bi bi-megaphone-fill"></i></div><h2>${escapeHtml(aviso.titulo)}</h2><div class="notice-copy">${escapeHtml(aviso.contenido).replaceAll("\n","<br>")}</div><button class="btn btn-primary mt-4" onclick="document.getElementById('generalNoticeModal').remove()">Entendido</button></div></div>`);
}
...
function roleOptions(selectedRole = "promotor") {
    const roles = currentUser.rol === "administrador" ? ["promotor","consulta","supervisor","rh","administrador"] : ["promotor","consulta","supervisor","rh"];
...
async function renderUserRegistration() {
    ...
    content.innerHTML = `
        ...
        <div class="card shadow-sm border-0"><div class="card-body p-4">
            ...
            <p class="text-muted">Obligatorias: nombre, usuario, correo, rol, activo y direccion. Opcionales: numero_empleado, sucursal, centro_costos, esquema_comisiones, esquema_contrato y observaciones.</p>
...
async function renderUserAlta() {
    ...
    content.innerHTML = `<div class="mb-4"><h1 class="h2 mb-1">Alta individual</h1><p class="text-muted mb-0">Todos los usuarios nuevos requieren una dirección activa.</p></div><div id="userActionArea"></div>`;
    showNewUserForm(directions);
}
...
async function renderUserConsultation() {
    ...
    const rows = users.map(user => `<tr>
        ...
        <td>${escapeHtml(user.direccion_nombre || "SIN ASIGNAR")}</td><td><span class="badge text-bg-secondary">${escapeHtml(user.rol)}</span></td>
...
function userFieldsHtml(directions, user = null, includeAccess = false) {
    return `<div class="row g-3">
        ...
        <div class="col-md-6"><label class="form-label">Dirección a cargo *</label><select id="userDirection" class="form-select" required>${directionOptions(directions,user?.direccion_id)}</select></div>
...
function collectUserData(includeAccess = false) {
    const data={nombre:document.getElementById("userName").value,correo:document.getElementById("userEmail").value,rol:document.getElementById("userRole").value,activo:document.getElementById("userActive").value==="true",direccion_id:Number(document.getElementById("userDirection").value),sucursal:document.getElementById("userBranch").value||null,centro_costos:document.getElementById("costCenter").value||null,esquema_comisiones:document.getElementById("commissionScheme").value||null,esquema_contrato:document.getElementById("contractScheme").value||null,observaciones:document.getElementById("userNotes").value||null,fecha_baja:document.getElementById("terminationDate").value||null,numero_empleado:document.getElementById("employeeNumber").value||null};
...
function showNewUserForm(directions) {
    document.getElementById("userActionArea").innerHTML=`<div class="card shadow-sm border-0"><div class="card-body p-4"><div id="newUserAlert"></div><form id="newUserForm">${userFieldsHtml(directions,null,true)}<button class="btn btn-primary mt-4"><i class="bi bi-person-check"></i> Crear usuario</button></form></div></div>`;
...
async function showUserEdit(id) {
    const [users,directions]=await Promise.all([apiFetch("/usuarios"),getActiveDirections()]); const user=users.find(item=>item.id===id); if(!user)return;
    document.getElementById("userActionArea").innerHTML=`<div class="card shadow-sm border-0 mb-4"><div class="card-body p-4"><div class="d-flex justify-content-between"><h2 class="h4">Modificar usuario</h2><button class="btn-close" onclick="closeUserAction()"></button></div><div id="editUserAlert"></div><form id="editUserForm">${userFieldsHtml(directions,user,false)}<button class="btn btn-primary mt-4"><i class="bi bi-save"></i> Guardar cambios</button></form></div></div>`;
