...
async function loadModule(name) {
    ...
    try {
        ...
        if (name === "catalogos") await renderCatalogs();
...
async function renderUserReport() {
    ...
    const rows = activeUsers.map(user => `<tr class="compact-report-row"><td class="report-name-cell"><span>${escapeHtml(user.nombre.toUpperCase())}</span></td><td>${escapeHtml((user.numero_empleado || "").toUpperCase())}</td><td class="two-line-cell"><span>${escapeHtml((user.direccion_nombre || "SIN ASIGNAR").toUpperCase())}</span></td><td class="two-line-cell"><span>${escapeHtml((user.responsable_nombre || "PENDIENTE").toUpperCase())}</span></td><td class="two-line-cell"><span>${escapeHtml((user.sucursal || "").toUpperCase())}</span></td><td>${escapeHtml(user.rol.toUpperCase())}</td></tr>`).join("");
...
async function renderCatalogs(){
 moduleName.textContent="CATÁLOGOS";
 ...
 content.innerHTML=`<div class="mb-4"><h1 class="h2">Administración de catálogos</h1><p class="text-muted">Cambie nombre, estado y orden. La clave interna permanece estable.</p></div><div class="accordion" id="catalogAccordion">${defs.map((def,i)=>`<div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button ${i?'collapsed':''}" data-bs-toggle="collapse" data-bs-target="#cat${i}">${def[1]} <span class="badge text-bg-primary ms-2">${data[i].length}</span></button></h2><div id="cat${i}" class="accordion-collapse collapse ${i===0?'show':''}"><div class="accordion-body"><div class="table-responsive"><table class="table align-middle"><thead><tr><th>Clave</th><th>Nombre</th><th>Orden</th><th>Estado</th><th></th></tr></thead><tbody>${data[i].map(x=>`<tr><td><code>${escapeHtml(x.clave)}</code></td><td><input id="cn-${def[0]}-${x.id}" class="form-control" value="${escapeAttribute(x.nombre)}"></td><td><input id="co-${def[0]}-${x.id}" type="number" class="form-control" style="width:90px" value="${x.orden}"></td><td><select id="ca-${def[0]}-${x.id}" class="form-select"><option value="true" ${x.activo?'selected':''}>Activo</option><option value="false" ${!x.activo?'selected':''}>Inactivo</option></select></td><td><button class="btn btn-sm btn-primary" onclick="saveCatalogItem('${def[0]}',${x.id})"><i class="bi bi-save"></i> Guardar</button></td></tr>`).join('')}</tbody></table></div></div></div></div>`).join('')}</div>`;
}
async function saveCatalogItem(catalog,id){try{await apiFetch(`/catalogos/${catalog}/${id}`,{method:'PUT',body:JSON.stringify({nombre:document.getElementById(`cn-${catalog}-${id}`).value,descripcion:null,orden:Number(document.getElementById(`co-${catalog}-${id}`).value),activo:document.getElementById(`ca-${catalog}-${id}`).value==='true'})});await renderCatalogs();}catch(error){alert(error.message)}}
